...
namespace IBSampleApp.ui
{
    ...
    {        
        ...
        public void HandleCommissionAndFeesMessage(CommissionAndFeesMessage message)
        {
            ...
            {
                if (((string)tradeLogGrid["executionIdExecColumn", i].Value).Equals(message.CommissionAndFeesReport.ExecId))
                {
        ...
        public void HandleExecutionMessage(ExecutionMessage message)
        {
            for (int i = 0; i < tradeLogGrid.Rows.Count; i++)
            {
                if (((string)tradeLogGrid["executionIdExecColumn", i].Value).Equals(message.Execution.ExecId))
                {
